...
export const StudentsTable = ({students} : Props) => {
    return (
        <table className="w-full border border-gray-600 rounded-md overflow-hidden">
            ...
            <tbody>
                {students.map(item => (
                    <tr key={item.id} className="text-gray-800 border-b border-gray-600">
                        ...
                        <td className="p-3">
                            {item.active && <div className="px-2 py-1 inline-block rounded-md border border-green-800 bg-green-500 text-white text-xs">Ativo</div>}
                            {!item.active && <div className="px-2 py-1 inline-block rounded-md border border-red-800 bg-red-500 text-white text-xs">Inativo</div>}
                        </td>
